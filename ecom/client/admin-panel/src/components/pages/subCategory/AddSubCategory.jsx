...
export default function AddSubCategory() {
  ...
  useEffect(() => {
    ...
    setImage(`https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/495px-No-Image-Placeholder.svg.png?20200912122019`)
          ...
          <form onSubmit={savesubCategory}>
            <div className='grid grid-cols-[30%_auto] gap-4'>
              ...
              <div className='w-full'>
                ...
                <select onChange={getValueorSetvalue} name='parentCategory' className=' w-full p-[10px] rounded-[10px] border border-gray-400 text-[14px] text-gray-400'>
                  <option >Select</option>
                  {
                    parentData.map((obj, index) => {
                      return (
                        <option
                          selected={obj._id == formValue.parentCategory} value={obj._id}
                ...
                <input type='text' onChange={getValueorSetvalue} name='subcategoryName' value={formValue.subcategoryName} placeholder='Sub Category Name' className=' w-full p-[8px] rounded-[8px] border border-gray-400 text-[14px] text-gray-400' />
                <h3 className='my-[10px] font-medium text-[14px]'>Order</h3>
                <input type='text' onChange={getValueorSetvalue} name='subcategoryOrder' value={formValue.subcategoryOrder} placeholder='Category Order' className=' w-full p-[8px] rounded-[8px] border border-gray-400 text-[14px] text-gray-400' />

              </div>
            </div>
            <button className='my-[20px] p-[10px_16px] rounded-[8px] text-white text-[16px] font-serif bg-sky-500 cursor-pointer duration-200 hover:bg-black hover:border hover:border-red-500'> {
